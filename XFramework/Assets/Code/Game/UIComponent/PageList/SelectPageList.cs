...
{
    public class SelectPageList : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler
    {
        ...
        public System.Action<int, GameObject> onItemEndAction = null;
        ...
        public float MaxPrevPos
        {
            get
            {
                ...
                if (direction == Direction.Horizontal)
                {
                    result = max.y * CellRect.x; //- viewPort.rect.width / 2;
                }
                else
                {
                    result = max.x * CellRect.y;// - viewPort.rect.height / 2;
                }
        ...
        private void CreateItem(int index)
        {
            ...
            item.localScale = new Vector3(  cellScale.x,  cellScale.y, 1);
        ...
        /// firsttarget 初始时的目标位置
        /// target UI组件的中心位置
        /// </summary>
        public bool NeedSelect = true;    
        ...
        public void OnEndDrag(PointerEventData eventData)
        {
            ...
            if (NeedSelect)
            {
                int selected_index = this.direction == Direction.Horizontal
                    ? (int) ((m_Rect.anchoredPosition.x - viewPort.rect.width / 2) / CellRect.x) + 1
                    :  Convert.ToInt32(Mathf.Round(m_Rect.anchoredPosition.y / CellRect.y + target));
                if (FirstSet)
                {
                    selected_index = firsttarget;
                    FirstSet = false;
                }
                
                if (selected_index == dataCount)
                {
                    selected_index--;
                }
                
                int _tmpCount = selected_index / PageScale - target;
                
                if (this.direction == Direction.Horizontal)
                {
                    var _offset = -_tmpCount * CellRect.x - viewPort.rect.width / 2;
                    m_Rect.anchoredPosition = new Vector2(_offset, m_Rect.anchoredPosition.y);
                }
                else
                {
                        var _offset = _tmpCount * CellRect.y; 
                        m_Rect.anchoredPosition = new Vector2(m_Rect.anchoredPosition.x,_offset);
                }
                
                HandleOffset();
                if (onItemEndAction != null)
                {
                    var go = m_InstantiateItems.Find(item =>
                        item.GetComponent<IPageListItem>().index == selected_index).gameObject;
                    this.onItemEndAction(selected_index, go);
                }
            }
        }
        ...
        {
            int _tmpCount = index - target;
            if (this.direction == Direction.Horizontal)
            {
                var _offset = -CellRect.x * (_tmpCount + 1);
                m_Rect.localPosition = new Vector2(_offset, m_Rect.anchoredPosition.y);
            }
            else
            {
                var _offset = CellRect.y * _tmpCount ;
                m_Rect.anchoredPosition = new Vector2(m_Rect.anchoredPosition.x, _offset);
            }
        ...
        public void OnDrag(PointerEventData eventData)
        {
            ...
            if (NeedSelect && dataCount == 1) return;
            //处理滚动边界
            float _offset = 0f;
            if (this.direction == Direction.Horizontal)
            {
            ...
            else
            {
                ...
                _offset = Mathf.Max(Mathf.Min(_offset, MaxPrevPos), viewPort.rect.height / 2 - target*CellRect.y);
